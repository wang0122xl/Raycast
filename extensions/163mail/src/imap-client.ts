...
const NETEASE_IMAP_HOST = "imap.163.com";
...
async function withClient<T>(operation: (client: ImapFlow) => Promise<T>): Promise<T> {
  ...
  try {
    try {
      ...
      throw new Error(
        `IMAP connection failed: ${errorMessage}\n\nPlease verify:\n- Your 163 email address is correct\n- The authorization code is correct (not your account password)\n- IMAP/SMTP service is enabled in 163 Mail settings`,
      );
    }
...
export async function listFolders(): Promise<Folder[]> {
  return withClient(async (client) => {
    ...
    // Inject virtual "Starred" folder after Inbox (starred mail = \Flagged flag)
