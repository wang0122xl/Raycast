...
export function MessageOpenInBrowserAction(props: {
  message: gmail_v1.Schema$Message;
  onOpen?: () => void;
  ...
  const urlOptions = { profileSegment: props.profileSegment };
  const url = isMailDraft(m) ? messageDraftEditUrl(profile, m, urlOptions) : messageThreadUrl(profile, m, urlOptions);
  if (!url) {
    return null;
  }
  const handle = async () => {
