...
type GmailWebUrlOptions = {
  profileSegment?: string;
};

export function gmailWebUrlBase(currentProfile: gmail_v1.Schema$Profile | undefined, options?: GmailWebUrlOptions) {
  const address = currentProfile?.emailAddress;
  return address ? `https://mail.google.com/mail/u/${options?.profileSegment ?? address}` : undefined;
}
...
export function messageDraftEditUrl(
  ...
  options?: GmailWebUrlOptions,
) {
  const prefix = gmailWebUrlBase(currentProfile, options);
