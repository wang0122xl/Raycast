...
export type OpenPinResult = {
  didOpen: boolean;
  openedInTerminal: boolean;
};

const EMPTY_RESULT_ACTION_PLACEHOLDERS = new Set([
  "alert",
  "copy",
  "createPin",
  "deletePin",
...
const hasEmptyResultActionPlaceholder = (target: string) => {
  return PinsPlaceholders.some(
    (placeholder) =>
      EMPTY_RESULT_ACTION_PLACEHOLDERS.has(placeholder.name) && target.match(placeholder.regex) != undefined,
  );
};
...
export const openPin = async (
  ...
  let result: OpenPinResult = { didOpen: false, openedInTerminal: false };
  try {
    if (pin.fragment) {
      ...
      result = { didOpen: true, openedInTerminal: false };
    } else {
        ...
        if (isPath) {
          ...
          if (target.match(/^[a-zA-Z](?![%])[a-zA-Z0-9+.-]+?:.*/g)) {
            ...
            if (pin.execInBackground) {
              ...
              result = { didOpen: true, openedInTerminal: true };
            }
          }
        }
      } else if (hasEmptyResultActionPlaceholder(targetRaw)) {
        result = { didOpen: true, openedInTerminal: false };
      }
    }
  } catch (error) {
