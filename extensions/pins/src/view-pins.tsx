...
import { Icon, List, ActionPanel, getPreferenceValues, LocalStorage } from "@raycast/api";
    ...
    if (args.launchContext?.pinID) {
      ...
      if (pin) {
        if (args.launchContext.action === PinAction.OPEN) {
          Promise.resolve(openPin(pin, preferences, localData as unknown as { [key: string]: unknown })).then(
            async (result) => {
